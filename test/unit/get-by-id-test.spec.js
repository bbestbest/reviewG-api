...
const { test,trait } = use('Test/Suite')('Get By ID Test')
...
test('should return user data from database', async ({ assert,client }) => {
  ...
  const userTest = await UserControllerUtil(UserModel).getByID(user_id)
...
test('should return admin data from database', async ({ assert,client }) => {
  ...
  const adminTest = await AdminControllerUtil(AdminModel).getByID(admin_id)
...
test('should return user score data from database', async ({ assert,client }) => {
  ...
  const userScoreTest = await UserScoreControllerUtil(UserScoreModel).getByID(user_score_id)
...
test('should return admin score data from database', async ({ assert,client }) => {
  ...
  const adminScoreTest = await AdminScoreControllerUtil(AdminScoreModel).getByID(admin_score_id)

  const response = await client.delete(`${urlAdminScores}/${admin_score_id}`).end()

  assert.isOk(adminScoreTest , 'Can not get Admin Score by ID')
...
test('should return comment data from database', async ({ assert,client }) => {
  ...
  const commentTest = await CommentControllerUtil(CommentModel).getByID(comment_id)
