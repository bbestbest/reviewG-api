...
class AdminScoreController {
    ...
    async store({request}) {
      const { story,gameplay,performance,graphic,admin_id } = request.body
      const { references } = request.qs
      ...
      const adminScore = await AdminScoreUtil(AdminScoreModel).create({story,gameplay,performance,graphic,overall,admin_id},references)
