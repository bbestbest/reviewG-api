...
class UserScoreController {
    ...
    async store({request}) {
        const { story,gameplay,performance,graphic,user_id,post_id} = request.body
        const { references } = request.qs
        ...
        const userScore = await UserScoreUtil(UserScoreModel).create({story,gameplay,performance,graphic,overall,user_id,post_id},references)
